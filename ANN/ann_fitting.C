...
for (int i=0; i<83; i++)
{
 data_k[i]= temp[i*468 + 1];
 data_QQ[i]= temp[i*468 + 2];
 data_xb[i]= temp[i*468 + 3];
 data_t[i]= temp[i*468 + 4];
 ...
 for (int j=0; j<36; j++)
  {
   data_phi = temp[i*468+j*13+5];
   data_F = temp[i*468+j*13+6];
   data_errF = temp[i*468+j*13+7];
   

   data_phi_rad = (data_phi/180)*pi;
   double bin_space = 0.049867;
   int bin_phi = (data_phi_rad-0)/bin_space + 1;
   hist_F[i]->SetBinContent(bin_phi, data_F);
   hist_F[i]->SetBinError(bin_phi, data_errF);  
